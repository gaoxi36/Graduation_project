...
class FFMPEG :
    ...
    def ffmpeg_push(self):
        subprocess.Popen("ffmpeg -re -i %s -vcodec %s -acodec %s -f flv rtmp://%s:%s/%s/%s"
                  %(self.__push_config['file'],self.__push_config['vcodec'],self.__push_config['acodec'],
                    self.__push_config['ip'],self.__push_config['port'],self.__push_config['app'],
                    self.__push_config['stream']), shell=True)



    def ffplay_rtmp_play(self):
        p = subprocess.Popen("ffplay rtmp://%s:%s/%s/%s"
                  %(self.__play_config['ip'],self.__play_config['port'],self.__play_config['app'],
                    self.__play_config['stream']), shell=True)
        time.sleep(5)
        NowTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        if (subprocess.Popen.poll(p) == 0):
            return ("\033[1;31;0m%s [%s]-[%s]-[%s]-[%s]-Rtmp:验证失败\033[1;32;0m"
                    %(NowTime, self.__play_config['ip'], self.__play_config['port'],
                      self.__play_config['app'], self.__play_config['stream']))
        else:
            p.kill()
            return ("\033[1;32;0m%s [%s]-[%s]-[%s]-[%s]-Rtmp:验证成功\033[1;32;0m"
                    % (NowTime, self.__play_config['ip'], self.__play_config['port'],
                       self.__play_config['app'], self.__play_config['stream']))
    ...
    def ffplay_m3u8_play(self):
        ...
        NowTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        if (subprocess.Popen.poll(p) == 0):
           return ("\033[1;31;0m%s [%s]-[%s]-[%s]-[%s]-M3u8:验证失败\033[1;32;0m"
                   % (NowTime, self.__play_config['ip'], self.__play_config['port'],
                       self.__play_config['app'], self.__play_config['stream']))
        else:
            p.kill()
            return ("\033[1;32;0m%s [%s]-[%s]-[%s]-[%s]-M3u8:验证成功\033[1;32;0m"
                    % (NowTime, self.__play_config['ip'], self.__play_config['port'],
                       self.__play_config['app'], self.__play_config['stream']))
    ...
    def ffplay_flv_play(self):
        ...
        NowTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        if (subprocess.Popen.poll(p) == 0):
            return("\033[1;31;0m%s [%s]-[%s]-[%s]-[%s]-Flv:验证失败\033[1;32;0m"
                   %(NowTime, self.__play_config['ip'], self.__play_config['port'],
                       self.__play_config['app'], self.__play_config['stream']))
        else:
            p.kill()
            return ("\033[1;32;0m%s [%s]-[%s]-[%s]-[%s]-Flv:验证成功\033[1;32;0m"
                    % (NowTime, self.__play_config['ip'], self.__play_config['port'],
                       self.__play_config['app'], self.__play_config['stream']))
